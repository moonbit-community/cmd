...
`-path` shell patterns, `-type`, `-empty`, `-size`, `-mtime`, `-newer`, depth
limits, `-depth`, `-prune`, negation, AND/OR composition, and newline or NUL
output. `-exec COMMAND ... ;` runs one direct child per match, while
