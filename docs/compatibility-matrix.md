...
| `env` | Coreutils 9.11 | partial | Assignment/unset and direct child launch exist; environment filtering is a deliberate policy difference and status/lookup need alignment. | Separate native inheritance compatibility from Wasm policy. |
...
| `printenv` | Coreutils 9.11 | partial | Selected/all variables and NUL records exist; environment visibility and option/status behavior need oracle coverage. | Align native environment and documented Wasm policy. |
...
| --- | --- | --- |
| The repository-added `reading standard input; send EOF to finish` message is not emitted by the original commands during an ordinary implicit stdin read. | `base64`, `cat`, `head`, `nl`, `paste`, `sha256sum`, `sh`, `sort`, `tail`, `tee`, `uniq`, `wc`, `xargs`, `xxd` | Remove it from the default compatibility path. Preserve silent blocking and EOF behavior; only emit an interactive message where the pinned upstream does. |
| Native child environments are filtered by current policy code even though `@env.get_env_vars()` is available. | `env`, `printenv`, `sh`, `make`, `xargs` and commands launched by them | Restore native upstream inheritance; keep Wasm denials explicit and target-specific. |
