...
Status: P0-P8 implementation shipped; audit ledger updated 2026-09-05
...
P0 through P8 are implemented in the current checkout and the unified runner
...
| `moon test --target all` | Native 98/98 and Wasm 89/89 passed; JS and Wasm-GC have no test entry | Package tests are green for configured targets |
...
| Unified manifest validation | 48 commands, 141 semantic cases | Fixture schema and pre-built artifact references are valid |
| Remote pinned oracle | Run `33948590271`, success | Linux pinned upstream comparison is green for the current HEAD [`6c3aee6`](https://github.com/moonbit-community/cmd/actions/runs/33948590271) |
...
phase5 17, P2 7, P3 23, P6 5, P7 7, and P8 12. The oracle suite executes all
141 manifest cases; the old claim of 66 cases was stale.
...
| --- | --- | --- | --- | --- | --- | --- |
| P0 | 2026-09-04 | `1953490` | Unified `compat`, `oracle`, and `policy` suites; pre-built artifact roots; retired tree removal | 141-case manifest validates; native and policy suites pass; remote oracle green | Local Docker oracle unavailable | Complete |
...
The immediate next work item is P8.1: add the gap-register probes using the
existing fixture schema, then rerun native, oracle, and policy gates. Only after
that audit passes should the support rows be promoted from a shipped bounded
profile to independently certified option families. New feature expansion is
lower priority than closing those evidence gaps.
